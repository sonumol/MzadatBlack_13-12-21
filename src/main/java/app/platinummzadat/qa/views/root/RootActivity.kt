...
import app.platinummzadat.qa.views.root.company.CompanyRegisterFragment1
...
import app.platinummzadat.qa.views.root.details.DetailsFragment
...
import kotlinx.android.synthetic.main.error_dashboard.*
            ...
            if (null != this) {
                if (this.contains("type")) {
                    ...
                    if ("notification" == intent?.extras?.getString("type")) {
                        ...
                        val type1 = intent?.extras?.getString("type1")!!



                        if(type1=="home/auction_details/"+itemId)
                        {
                            if (-1 != itemId) {
                                dashboardFragment.refresh = true
                                replaceFragment(
...
//                        Toast.makeText(applicationContext,""+type1,Toast.LENGTH_LONG).show()
                    }
...
//                        }
                        ...
                        dashboardFragment.refresh = true
                        replaceFragment(CompanyRegisterFragment1(), R.id.container, true)
                    }
                }
    ...
    override fun onSelectDashboardItem(item: DashboardItemModel) {
        ...
        if(item.name==getString(R.string.direct_Purchase) )
        {
    ...
    override fun onSelectNavItem(navItem: MzNav) {
        ...
        when (navItem) {
            ...
            REGISTER_AS_COMPANY -> {
                ...
                val sharedPreference1=  getSharedPreferences("user_international",Context.MODE_PRIVATE)
                var  user_international = sharedPreference1.getInt("user_international",0)
                //Toast.makeText(applicationContext,"k"+user_international,Toast.LENGTH_LONG).show()
                ...
                if (user_international==1)
                {
                    root.longSnackbar(getString(R.string.onlyqataruser), getString(R.string.cancel)) {
...
//                       Snackbar.LENGTH_LONG).setAction("Action",null).show()
            ...
            LOGOUT -> {
                if (-1 != currentUserId) {
                    ...
                    finish()
                    val preferences: SharedPreferences=this!!.getSharedPreferences("user_id", 0)
                    preferences.edit().remove("user_id").commit()
                    val preferences2: SharedPreferences=this!!.getSharedPreferences("path", 0)
                    preferences2.edit().remove("path").commit()
                } else {
...
//        val editor:SharedPreferences.Editor =  sharedPreferences.edit()
////        val name:String =radioButton.text.toString()
//        editor.putInt("id", international)
//        editor.apply()
//        editor.commit()
//        Toast.makeText(MApp.applicationContext(),"ff",Toast.LENGTH_LONG)
